...
class MultiBoxLoss2(nn.Module):
    ...
    def forward(self, conf_pred, loc_pred, conf_true, loc_true):
        ...
        with torch.no_grad():
            ...
            mask = hard_negative_mining(loss, conf_true, self.neg_pos_ratio)
        ...
        g_loc_pred = cxcy_to_gcxgcy(loc_pred, self.default_box)  # (8732, 4), (gcx, gcy, gw, gh)
        pos_mask = conf_true > 0
        # positive인 box들만(BG아닌 box들) 사용하는게 맞음
        g_loc_pred = g_loc_pred[pos_mask, :]

        loc_true = loc_true[pos_mask, :]
        loc_loss = F.smooth_l1_loss(g_loc_pred.view(-1, 4), loc_true.view(-1, 4), reduction='sum')
        num_pos = loc_true.size(0)
